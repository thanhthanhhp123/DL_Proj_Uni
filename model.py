from layers import Convolutional, Pooling, FullyConnected, Dense, regularized_cross_entropy, lr_schedule
...
class Network:
    ...
    def build_model(self, dataset_name = None):
        if dataset_name == 'mnist':
            ...
            # self.add_layer(FullyConnected(name='fullyconnected', nodes1=7744, nodes2=256, activation='relu'))
            # self.add_layer(Dense(name='dense', nodes=256, num_classes=10))
            self.add_layer(Convolutional(name='conv1', num_filters=8, stride=2, size=3, activation='relu'))
            self.add_layer(Convolutional(name='conv2', num_filters=8, stride=2, size=3, activation='relu'))
            self.add_layer(Dense(name='dense', nodes=1352, num_classes=2))
    ...
    def predict(self, X):
        predictions = []
        for i in range(len(X)):
            tmp_output = self.forward(X[i], plot_feature_maps=0)       # forward propagation
            predictions.append(np.argmax(tmp_output))
        return predictions